<?php
  ...
  if (empty($new_password)){
    die(format_result("ERROR_EMPTY_NEW_PASSWORD"));
  }
  ...
  $queries[] = $usr;
  $salt = trim($usr["salt"]);
  $query = "UPDATE tg_users SET password = MD5('$new_password:$salt'), salt = '$salt' WHERE \"ID\" = $user_id";
  $queries[] = $query;
  ...
  if ($required_duration > 0){
	...
	$usr["rang"]["next_name"] = trim($next_rang["name"]);
